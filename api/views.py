...
class PrestamoFilter(django_filters.FilterSet):
    Fecha_prestamo =  filters.DateTimeFilter(field_name='Fecha_prestamo')
    Fecha =  filters.DateTimeFromToRangeFilter(field_name='Fecha_prestamo')
...
class LibroActiveList(generics.ListCreateAPIView):
    permission_classes = (AllowAny,)
    queryset = Libro.objects.raw('SELECT r.id FROM libros_libro r WHERE NOT EXISTS(SELECT r.id FROM prestamos_detalleprestamo d WHERE r.id = d."Recurso_detalle_id"  AND r."Estado_libro" = %s AND d."Estado" = %s) ORDER BY r.id' %("'ACTIVO'", "'PRESTADO'") )
    serializer_class = LibroSerializer
...
class TipolibroList(generics.ListCreateAPIView):
    queryset = Tipolibro.objects.all()
    serializer_class = Tipo_libroSerializer

class DetalleTipolibroDetail(generics.RetrieveUpdateDestroyAPIView):
    queryset = Tipolibro.objects.all()
    serializer_class = Tipo_libroSerializer
