...
@login_required
def codigo_barras(request,Id_libro):
    r = get_object_or_404(Libro,id=Id_libro)
    context = admin.site.each_context(request)
    context.update({
        'r': r,
    })
...
@login_required
def libro_reporte(request,Id_libro):
    r = get_object_or_404(Libro,id=Id_libro)
    context = admin.site.each_context(request)
    context.update({
        'r': r,
    })
    return  render(request,'core/reporte_libro.html',context)
